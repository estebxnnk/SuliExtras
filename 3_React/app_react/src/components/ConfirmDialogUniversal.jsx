...
// Confirmación universal con el mismo diseño/animaciones de ConfirmDialogWithLogo
function ConfirmDialogUniversal({
  open,
  action,
  data,
  onClose,
    ...
    switch (act) {
      ...
      case 'rol':
        return {
          icon: InfoIcon,
          color: '#7b1fa2',
          bgGradient: 'linear-gradient(135deg, rgba(255,255,255,0.98) 0%, rgba(245,240,255,0.98) 100%)',
          borderColor: '#7b1fa2',
          shadowColor: 'rgba(123,31,162,0.3)',
          title: '¡Confirmar Cambio de Rol!',
          message: '¿Estás seguro que deseas CAMBIAR el rol?'
        };
      default:
        return {
          icon: InfoIcon,
  ...
  const ActionIcon = config.icon || WarningIcon;

  const effectiveTitle = title || config.title;
  const effectiveMessage = message || config.message;
    ...
    <Fade in={open} timeout={500}>
      ...
      >
        ...
        <style>{`
          ...
          @keyframes successScale { 0% { transform: scale(0); opacity: 0; } 50% { transform: scale(1.2); } 100% { transform: scale(1); opacity: 1; } }
...
export default ConfirmDialogUniversal;
